...
using Connection = r::Connection<socket_t>;

class BoostBredis {

public:
	boost::system::error_code ec;
	r::Connection<socket_t> *c;
	...
	void SetBredis(std::string hostAddr="127.0.0.1", int port=6379, std::string requirepass= "123456")
	{
		...
		socket = new socket_t(ioContext, endpoint.protocol());
		socket->connect(endpoint);
		c = new r::Connection<socket_t>(std::move(*socket));
